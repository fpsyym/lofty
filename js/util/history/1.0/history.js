 ...
 *
 * Git project: https://github.com/devote/HTML5-History-API
 */
  
!function(definition) {
    if (typeof define === 'function'/* && define.amd*/) {
        // Register as an AMD module.
        define('lofty/util/history/1.0/history', [], definition);
    } else {
        // Browser globals
        definition();
    }
	...
	// save the object 'state' in interface 'History'
    var stateObjectInHistory = null;
    ...
    var eventsList = {};
    // force HTML5 browser use HTML4 hash history
    var bForceHash = false;
    /**
    ...
    var historyDescriptors = {
		...
		init:function(config){
			return initialize(config && config.forceHash);
		},
    ...
    /**
     * Force to use HTML4 hash way
     *
     * @param hash
     */
	function disableHtml5API() {
		bForceHash = true;
		historyPushState = null;
		historyReplaceState = null;
		isSupportHistoryAPI = false;
		isSupportStateObjectInHistory = false;
	}
    ...
    function initialize(forceHash) {
		...
		if (!isSupportStateObjectInHistory) {
			...
			var descriptor = prepareDescriptorsForObject(stateDescriptors, "state", stateDescriptors["state"]);
			// try to set the descriptor object
			if (!redefineProperty(historyObject, "state", descriptor)) {
