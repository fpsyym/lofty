   ...
   'use strict';
	...
	var Attribute = Class({
		...
		initOptions: function(config) {
			...
			if (config) {
				mergeConfig(this.options, config);
			}
			
			setSetterAttrs(this, this.options, config);
		},
			...
			for (key in options) {
				...
				if (!this.__initializingAttrs && !isEqual(prev, val)) {
					...
					changed[key] = [val, prev];
						...
						this.trigger(key + 'Change' , val, prev, key);
					}
				}
			}
			return this;
		...
		change: function() {
			...
			if (changed) {
				for (var key in changed) {
					...
					this.trigger(key + 'Change', args[0], args[1], key);
					}
				}
	...
	function mergeInheritedAttrs(options, instance) {
		...
		var chains = getPrototypeChain(instance);
